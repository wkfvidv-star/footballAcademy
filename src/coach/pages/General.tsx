        ...
        <div className="space-y-10 animate-in fade-in pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <button className="h-14 px-8 rounded-2xl bg-white/5 hover:bg-white/10 text-white flex items-center gap-3 border border-white/10 transition-all active:scale-95 group font-black uppercase tracking-widest text-xs">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-6">
                    <div className="bg-zinc-900 border border-white/5 rounded-[48px] p-10 min-h-[450px] flex flex-col relative overflow-hidden group hover:border-blue-500/20 transition-all">
                        <div className="absolute -right-24 -top-24 w-96 h-96 bg-blue-500/5 rounded-full blur-[100px] group-hover:bg-blue-500/10 transition-all"></div>
                        ...
                        <div className="flex justify-between items-center mb-10 z-10">
                            <div>
                                ...
                                <p className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.2em] mt-1 italic">{t('coach_general.cumulative_kpi')}</p>
                            </div>
                            <select className="bg-white/5 border border-white/10 rounded-2xl px-5 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 focus:outline-none focus:border-blue-500/50 transition-colors">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-zinc-900 border border-white/5 rounded-[40px] p-8">
                        ...
                        <div className="space-y-4">
                            {loadMetrics?.riskLevel && (loadMetrics.riskLevel === 'high' || loadMetrics.riskLevel === 'critical') && (
                                <div className="p-5 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-start gap-4 animate-pulse">
                                    <div className="p-2 bg-red-500/20 rounded-xl mt-1"><AlertTriangle className="w-4 h-4 text-red-500" /></div>
                                    <div>
                                        <div className="text-sm font-black text-white uppercase italic">{t('coach_general.high_injury_risk')}</div>
                                        <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide mt-1">
                            ...
                            <div className="p-5 bg-amber-500/5 border border-amber-500/10 rounded-2xl flex items-start gap-4">
                                ...
                                <div>
                                    <div className="text-sm font-black text-white uppercase italic">{t('coach.dashboard.performance_drop')}</div>
                                    <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide mt-1">Midfield Unit (-15%)</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 border border-white/5 rounded-[40px] p-8">
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <button className="text-[10px] font-black text-blue-500 hover:underline uppercase tracking-widest">{t('common.viewAll')}</button>
                        </div>
                        <div className="space-y-4">
                            {[1, 2, 3].map((i) => (
                                <div key={i} className="flex items-center gap-4 p-4 hover:bg-white/2 rounded-2xl transition-all border border-transparent hover:border-white/5 cursor-pointer group">
                                    <div className="w-12 h-12 bg-zinc-800 rounded-[18px] flex items-center justify-center text-[10px] font-black text-zinc-500 group-hover:bg-blue-600/20 group-hover:text-blue-500 transition-all border border-white/5">
                                        P{i}
                                    </div>
                                    <div>
                                        <div className="text-[11px] font-black text-white uppercase italic">{t('coach.dashboard.eval_completed')}</div>
                                        <div className="text-[9px] text-zinc-500 font-bold uppercase tracking-tight mt-0.5">
