        ...
        >
            <div className="space-y-8">
                ...
                <div className="space-y-3">
                    ...
                    <div className="h-3 w-full bg-zinc-800 rounded-full flex overflow-hidden border border-white/5">
                ...
                {stats.recommendations.length > 0 && (
                    <div className="bg-amber-500/5 border border-amber-500/20 p-5 rounded-2xl space-y-3 group hover:bg-amber-500/10 transition-colors">
                        ...
                        <ul className="space-y-2">
                            {stats.recommendations.map((rec, i) => (
                                <li key={i} className="text-xs text-zinc-400 font-medium leading-relaxed flex items-start gap-2">
            ...
            {showAddForm && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6 animate-in fade-in transition-all">
                    <div className="bg-zinc-900 border border-white/10 p-8 rounded-[40px] w-full max-w-md shadow-2xl shadow-black">
                        <h3 className="text-2xl font-black text-white mb-6 uppercase italic tracking-tight">{t('nutrition_module.log_meal')}</h3>
                        <form onSubmit={handleAddMeal} className="space-y-6">
                            <div className="space-y-2">
                                <label className="block text-[10px] text-zinc-500 uppercase font-black tracking-widest px-1">{t('nutrition_module.meal_type')}</label>
                                <select name="type" className="w-full bg-zinc-800/50 border border-white/10 rounded-2xl p-4 text-white outline-none focus:border-primary/50 transition-colors text-sm font-bold">
                            ...
                            <div className="space-y-2">
                                <label className="block text-[10px] text-zinc-500 uppercase font-black tracking-widest px-1">{t('nutrition_module.description')}</label>
                                <input name="description" required className="w-full bg-zinc-800/50 border border-white/10 rounded-2xl p-4 text-white outline-none focus:border-primary/50 transition-colors text-sm font-bold" placeholder="e.g. Chicken Pasta" />
                            </div>
                            ...
                            <div className="flex gap-4 mt-8 pt-4 border-t border-white/5">
                                <button type="button" onClick={() => setShowAddForm(false)} className="flex-1 py-4 text-zinc-500 font-black uppercase text-xs tracking-widest hover:text-white transition-colors">{t('nutrition_module.cancel')}</button>
                                <button type="submit" className="flex-1 py-4 bg-primary text-zinc-900 font-black rounded-2xl hover:scale-[1.02] active:scale-95 transition-all uppercase text-xs tracking-widest">{t('nutrition_module.save_meal')}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
function StatCard({ label, value, target, unit }: { label: string, value: number, target?: number, unit: string }) {
    return (
        <div className="bg-white/5 border border-white/5 p-4 rounded-2xl group hover:border-primary/20 transition-all">
            <div className="text-[10px] text-zinc-500 uppercase font-black tracking-widest mb-2 px-1">{label}</div>
            <div className="flex items-baseline gap-1.5 px-1">
                <span className="text-xl font-black text-white italic">{value}</span>
                <span className="text-[10px] text-zinc-500 uppercase font-bold">{unit}</span>
            </div>
            {target && (
                <div className="mt-3 h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden border border-white/5 mx-0.5">
                    <div
